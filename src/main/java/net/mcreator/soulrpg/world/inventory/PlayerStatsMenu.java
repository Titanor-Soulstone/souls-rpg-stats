...
package net.mcreator.soulrpg.world.inventory;
...
public class PlayerStatsMenu extends AbstractContainerMenu implements Supplier<Map<Integer, Slot>> {
	...
	public PlayerStatsMenu(int id, Inventory inv, FriendlyByteBuf extraData) {
		super(SoulRpgModMenus.PLAYER_STATS.get(), id);
		...
		for (int si = 0; si < 3; ++si)
			for (int sj = 0; sj < 9; ++sj)
				this.addSlot(new Slot(inv, sj + (si + 1) * 9, 47 + 8 + sj * 18, 36 + 84 + si * 18));
		for (int si = 0; si < 9; ++si)
			this.addSlot(new Slot(inv, si, 47 + 8 + si * 18, 36 + 142));
	}
